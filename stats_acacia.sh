...
for og in $file_list; do
    ...
    echo "$name" | while read -r line; do
        ...
        if [[ $line == *.hifi.* ]]; then
            ...
            path=pawsey0812:oceanomics-mitogenomes/$og"$line"
            stats=$(rclone cat "$path"mtdna/ --include "*get_org.log.txt" --max-depth 1 | grep "Result status of animal_mt: " | tail -n 1 | awk -F 'animal_mt: ' '{print $NF}')
        fi                
           
        # Check if there is a genbank file
        genbank=$(rclone cat "$path"genbank/ --include "*.{fa,fasta}" --max-depth 1 | grep ">" | sed 's/>//g')

        # Count the length of the sequence
        length=$(rclone cat "$path"mtdna/ --include "*.{fa,fasta}" --max-depth 1 | grep -v "^>" | tr -d '\n' | awk '{print length}')
        length_emma=$(rclone cat "$path"emma/ --include "*.{fa,fasta}" --max-depth 1 | grep -v "^>" | tr -d '\n' | awk '{print length}')
